...
We first computed LMMs for latency, excess path length, excess distance to goal and initial rotation in training trials and tested for changes across trials (learning curves) and age differences (see statistics in Table 2; Figure 2A). 
...
Participants also showed a significant decrease in excess distance to goal across trials (`r gsub(".*, ", "", apa_lmm.train_distance$statistic$trial_in_block)`), meaning that they searched closer to the goal location after the first trial. Again, there was a general age difference, indicating that 6-8YO children searched further away from the goal location than 9-11YO children (`r apa_post.train_distance_group$statistic$YoungKids_OldKids`) and adults (`r apa_post.train_distance_group$statistic$YoungKids_YoungAdults`). 
...
Taken together, participants from all age groups were able to significantly improve their navigation behavior across training trials. Younger participants had a less efficient navigation behavior, which was characterized by slower navigation (latency), less direct navigation (excess path length), less focused search behavior (excess distance to goal) and a lack of visual exploration during orientation (initial rotation). Navigation differences were pronounced between 6-8YO children in comparison to 9-10YO children and adults, but less evident when comparing 9-10YO children to adults.
